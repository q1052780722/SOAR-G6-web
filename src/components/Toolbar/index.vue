...
export default {
  data() {
    return {
	  //剧本相关	
	  //TODO 将请求中的json playbook 赋值给this.json this.playbook
	  json:{}, 
	  playbook:"",
  ...
  mounted(){
	  ...
	  var that=this;
	  this.$axios.get("http://192.168.12.137:8080/playbook/1")
	  		.then(function(response){
	  				// var json=JSON.parse(response);
	  				// console.log(response.data.json);
	  				// that.json=JSON.parse(response.data.json);
